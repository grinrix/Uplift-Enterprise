...
func _resolve_mail_input(btn, input_field, task):
	var player_text = input_field.text.strip_edges() # Usuwa spacje
	
	# Ignoruj wielkość liter (Service == SERVICE)
	if player_text.to_upper() == task["correct_answer"].to_upper():
		Global.add_score(task["score"])
		btn.text = "CORRECT"
		btn.modulate = Color.GREEN
		Global.add_score(50) # Bonus za poprawny kod
	else:
...
# --- RESZTA FUNKCJI (SKLEP I TERMINAL BEZ ZMIAN) ---
func generate_shop():
